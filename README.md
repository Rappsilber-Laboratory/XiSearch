...
"Small scale" presets refer to a small search with crosslinker modifications (amidated, hydrolysed, crosslinks within a peptide) searched as variable modifications only on linear peptides rather than peptide pairs. For crosslinkers using NHS-ester chemistry (DSS, BS3, DSBU, BS2G), S/T/Y is considered a side reaction and a score penalty is applied to the match relative to matching spectra crosslinked to K or Nterm. This is recommended for searches with many (>200) proteins in the database. "Large scale" presents perform a large search with crosslinker modifications treated as variable modifications on every peptide.
