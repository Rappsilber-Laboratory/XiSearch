...
public class PeakCoOccurenceBined {
    ...
    public void setTargetPeaks(Collection<Double> TargetPeaks) {
        ...
        this.TargetPeaks.addAll(TargetPeaks);
    }
    
    /**
     * @return the m_CoOccurence
     */
    public CoOccurence getCoOccurence() {
        return m_CoOccurence;
    }
     ...
     */
    public void setPeakList(SpectraAccess msm) {
        this.msm = msm;
    }
     ...
     */
    public void setFilter(ScanFilteredSpectrumAccess filter) {
        this.filter = filter;
    }

    /**
     * @return the m_peakCentredBins
     */
    public boolean isPeakCentredBins() {
        return m_peakCentredBins;
    }

    /**
     * @param m_peakCentredBins the m_peakCentredBins to set
     */
    public void setPeakCentredBins(boolean m_peakCentredBins) {
        this.m_peakCentredBins = m_peakCentredBins;
    }
    ...
    public void resultToStream(PrintStream out) {
        ...
        for (int mz1 = 0; mz1 < m_targetMz.length;mz1++) {
            out.print(m_targetMz[mz1] + "," + (m_foundPeaks[mz1][0] + (double)m_missedPeaks[mz1][0]));
            for (int mz2 = 0; mz2 < m_targetMz.length;mz2++) {
                out.print(",");
                out.print(m_foundPeaks[mz1][mz2] / (m_foundPeaks[mz1][mz2] + (double)m_missedPeaks[mz1][mz2]));

                if (!occurences.contains(m_foundPeaks[mz1][mz2])) {
                    occurences.add(m_foundPeaks[mz1][mz2]);
                }
            }
            out.print("\n");
        }
        ...
        for (int mz1 = 0; mz1 < m_targetMz.length;mz1++) {
            ...
            for (int mz2 = 0; mz2 < m_targetMz.length;mz2++) {
                out.print(",");
                out.print(m_foundPeaks[mz1][mz2]);
            }
            out.print("\n");
        }
        out.print("\n");
        ...
        out.print("relative to max occurence\n");
        ...
        for (int mz1 = 0; mz1 < m_targetMz.length;mz1++) {
            ...
            for (int mz2 = 0; mz2 < m_targetMz.length;mz2++) {
                out.print(",");
                int o = m_foundPeaks[mz1][mz2];
                double p = occurenceIndex.get(o) / max;
                out.print(p);
            }
            out.print("\n");
        }
    ...
    public static void main(String[] args) throws ParseException {
        PeakCoOccurenceBined po = new PeakCoOccurenceBined();
        String PeakMSM = null;
            ...
            for (int i = 0 ; i< args.length; i++) {
                ...
                if (arg.startsWith("--tolerance=")){
                    ...
                    try {
                        ...
                        Logger.getLogger(PeakCoOccurenceBined.class.getName()).log(Level.SEVERE, "could not open output-file " + a[1], ex);
                    ...
                    try {
                        ...
                        Logger.getLogger(PeakCoOccurenceBined.class.getName()).log(Level.SEVERE, "could not open output-file " + args[i], ex);
                        printUsage(System.err);
                        return;
                    }
        ...
        if (PeakList == null) {
            ...
            try {
                ...
                Logger.getLogger(PeakCoOccurenceBined.class.getName()).log(Level.SEVERE, "File not found", ex);
                printUsage(System.err);
                return;
            } catch (IOException ex) {
                Logger.getLogger(PeakCoOccurenceBined.class.getName()).log(Level.SEVERE, "Error accessing file", ex);
                printUsage(System.err);
                return;
            }
        }
        ...
        if (peaks != null) {
            ...
            try {
                ...
                Logger.getLogger(PeakCoOccurenceBined.class.getName()).log(Level.SEVERE, null, ex);
                return;
            } catch (IOException ex) {
                Logger.getLogger(PeakCoOccurenceBined.class.getName()).log(Level.SEVERE, null, ex);
                return;
