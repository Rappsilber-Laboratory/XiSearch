...
public class SimpleXiProcessMultipleCandidates_MGCWriteOut extends SimpleXiProcessLinearIncluded{
    ...
    public SimpleXiProcessMultipleCandidates_MGCWriteOut(SequenceList fasta, AbstractSpectraAccess input, ResultWriter output, RunConfig config, StackedSpectraAccess filter) {
        ...
        try {
            ...
            Logger.getLogger(SimpleXiProcessMultipleCandidates_MGCWriteOut.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    public SimpleXiProcessMultipleCandidates_MGCWriteOut(File fasta, AbstractSpectraAccess input, ResultWriter output, RunConfig config, StackedSpectraAccess filter) {
        super(fasta, input, output, config, filter);
        try {
            this.mgcOut = new MSMWriter(new File("/tmp/xi1alpha.mgf"), "", "", "");
            mgcOut.writeHeader();
...
//                        matches[0].setValidated(true);
            ...
            Logger.getLogger(SimpleXiProcessMultipleCandidates_MGCWriteOut.class.getName()).log(Level.SEVERE, "("+Thread.currentThread().getName()+") Error while processing spectra", e);
