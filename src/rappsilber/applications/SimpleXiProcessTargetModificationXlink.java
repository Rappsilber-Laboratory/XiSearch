 ...
 */
public class SimpleXiProcessTargetModificationXlink extends SimpleXiProcessLinearIncluded{
...
//    private HashMap<AminoAcid,ModificationLookup> m_KnownModifications;
    ...
    public SimpleXiProcessTargetModificationXlink(File fasta, AbstractSpectraAccess input, ResultWriter output, RunConfig config, StackedSpectraAccess filter) {
    ...
    public SimpleXiProcessTargetModificationXlink(SequenceList fasta, AbstractSpectraAccess input, ResultWriter output, RunConfig config, StackedSpectraAccess filter) {
        super(fasta, input, output, config, filter);
...
//                        System.out.println("reduced to Last MGX index : " + lastMGXIndex);
                    ...
                    for (int mgxID = 0; mgxID <= lastMGXIndex;mgxID++) {
                        ...
                        evaluateMatch(spectra.cloneComplete(), ap, bp, cl, 0, scanMatches, mgcScore, mgcDelta, mgcShiftedDelta, alphaMGC, 0, mgxScore, mgxDelta, mgxID, false);
                    }
                    spectra.free();
...
//            brw.waitForFinished();
            ...
            Logger.getLogger(SimpleXiProcessTargetModificationXlink.class.getName()).log(Level.SEVERE, "Error while processing spectra", e);
